angular
    .module('myApp')
    .controller('createPrivateeditornoteCtl', ['$scope', '$state', '$rootScope', '$http','PrivateEditorNote',
        function ($scope, $state, $rootScope, $http , PrivateEditorNote) {
            ...
            $scope.create = () => {
                ...
                if(isTrue) {
                    PrivateEditorNote.create({ title , fullDesc })
                    .$promise.then((res) => {
    ...
    .controller('manageeditornotePrivateCtl', ['$scope', '$state', '$rootScope', '$http', 'PrivateEditorNote',
        function ($scope, $state, $rootScope, $http, PrivateEditorNote) {
            ...
            $scope.getAllEditorNote = () => {
                $scope.editorNoteList = [];
                PrivateEditorNote.find({ filter : { where : { status : 'new' } } }).$promise.then((res) => {
            ...
            $scope.confirmDelete = () => {
                ...
                if (data && data.id) {
                    PrivateEditorNote.deleteById({ id: data.id }).$promise.then((res) => {
                        $scope.getAllEditorNote();
        ...
        .controller('editeditornoteCtl', ['$scope', '$state', '$rootScope', '$http', 'PrivateEditorNote',
        function ($scope, $state, $rootScope, $http, PrivateEditorNote) { 
            toastMsg = (isVaild, msg) => {
                if (isVaild)
                    toastr.success(msg);
