...
module.exports = function(Privateeditornote) {

    Privateeditornote.observe('before save', function (ctx, next) {
        if (ctx.isNewInstance && ctx.instance) {
            Privateeditornote.find({ where: { status: "new" } }, (err, res) => {
                if (err) next();
                else {
                    if (res.length) {
                        res.forEach((val, i) => {
                            if (val.id) Privateeditornote.upsertWithWhere({ id: val.id }, { status: 'rejected' })
    ...
    Privateeditornote.getFindAll = (params, cb) => {
     ...
     Privateeditornote.getFindOne = (params, cb) => {
        let isCallBack = (isSuccess = false, message = "Please try again", result = {}, error = {}) => cb(null, { isSuccess, message, result, error });
         if (params) {
             let { editorId } = params;
             if (editorId) {
                Privateeditornote.find({ where: { id: editorId } }, (err, res) => {
                     if (err) isCallBack(false, 'please try again', {}, err);
     ...
     Privateeditornote.remoteMethod('getFindAll', {
     ...
     Privateeditornote.remoteMethod('getFindOne', {
